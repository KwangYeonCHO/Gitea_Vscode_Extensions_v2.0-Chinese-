...
class GiteaAPI {
    ...
    async getUserRepos() {
        try {
            ...
            const response = await axios_1.default.get(`${serverUrl}api/v1/user/repos`, {
                headers: {
    ...
    async createRepo(name, description, isPrivate = false) {
        try {
            // 确保服务器地址以 / 结尾
            const serverUrl = this.serverUrl.endsWith('/') ? this.serverUrl : `${this.serverUrl}/`;
            const response = await axios_1.default.post(`${serverUrl}api/v1/user/repos`, {
                name,
                description,
                private: isPrivate
            }, {
    ...
    async getPullRequests(owner, repo) {
        try {
            const response = await axios_1.default.get(`${this.serverUrl}/api/v1/repos/${owner}/${repo}/pulls`, {
                headers: {
                    'Authorization': `token ${this.token}`
                }
            });
    ...
    async createPullRequest(owner, repo, title, body, head, base) {
        try {
            const response = await axios_1.default.post(`${this.serverUrl}/api/v1/repos/${owner}/${repo}/pulls`, {
                title,
                body,
                head,
                base
    ...
    async repoExists(name) {
        try {
            ...
            const serverUrl = this.serverUrl.endsWith('/') ? this.serverUrl : `${this.serverUrl}/`;
            // 先获取用户信息以获取用户名
            const userResponse = await axios_1.default.get(`${serverUrl}api/v1/user`, {
                headers: {
                    'Authorization': `token ${this.token}`
                }
            });
            ...
            const response = await axios_1.default.get(`${serverUrl}api/v1/repos/${username}/${name}`, {
                headers: {
                    'Authorization': `token ${this.token}`
                },
                validateStatus: function (status) {
    ...
    async deleteRepo(owner, repo) {
        try {
            // 确保服务器地址以 / 结尾
            const serverUrl = this.serverUrl.endsWith('/') ? this.serverUrl : `${this.serverUrl}/`;
            const response = await axios_1.default.delete(`${serverUrl}api/v1/repos/${owner}/${repo}`, {
                headers: {
                    'Authorization': `token ${this.token}`
                }
            });
...
//# sourceMappingURL=giteaApi.js.map