...
class RepoManager {
    ...
    async createRepository(config) {
        try {
            ...
            const response = await axios_1.default.post(`${server.url}/api/v1/user/repos`, {
                name: config.name,
        ...
        try {
            ...
            return await vscode.window.withProgress({
                ...
                return new Promise((resolve, reject) => {
                    ...
                    // 创建子进程
                    const process = exec(`git clone "${cloneUrl}" "${targetPath}"`, { maxBuffer: 10 * 1024 * 1024 } // 增加缓冲区大小
                    );
                    // 用于模拟进度
                    let progressValue = 0;
    ...
    async listRepositories() {
        try {
            ...
            const response = await axios_1.default.get(`${server.url}/api/v1/user/repos`, {
                headers: {
                    'Authorization': `token ${server.token}`
                }
            });
    ...
    async deleteRepository(owner, repo) {
        try {
            ...
            const response = await axios_1.default.delete(`${server.url}/api/v1/repos/${owner}/${repo}`, {
                headers: {
                    'Authorization': `token ${server.token}`
                }
            });
