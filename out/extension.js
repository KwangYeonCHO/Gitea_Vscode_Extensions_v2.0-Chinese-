...
const visibilityManager_1 = require("./visibilityManager");
// Git 操作类
class GitOperations {
    constructor() {
    ...
    async initRepo() {
        try {
            ...
            const workspaceFolder = vscode.workspace.workspaceFolders[0].uri.fsPath;
            // 使用指定的工作目录初始化Git实例
            const tempGit = (0, simple_git_1.default)(workspaceFolder);
            // 检查是否已经是Git仓库
    ...
    async addRemote(name, url) {
        try {
            ...
            try {
                ...
                if (existingUrl) {
                    // 如果远程仓库已存在，询问是否更新
                    const update = await vscode.window.showWarningMessage(`远程仓库 ${name} 已存在，URL为: ${existingUrl}\n是否更新为新的URL: ${url}？`, { modal: true }, '更新', '取消');
                    if (update === '更新') {
                        // 删除现有的远程仓库
    ...
    // 设置上游分支
    async setUpstreamBranch(branchName, remoteName = 'origin') {
        try {
            if (!this.isGitRepo && !(await this.initialize())) {
                throw new Error('无法初始化Git仓库');
            }
...
// 使用 exec 克隆仓库
async function cloneWithExec(url, path) {
    return vscode.window.withProgress({
        ...
        return new Promise((resolve, reject) => {
            ...
            // 创建子进程
            const process = exec(`git clone "${url}" "${path}"`, { maxBuffer: 10 * 1024 * 1024 } // 增加缓冲区大小
            );
            // 用于模拟进度
            let progressValue = 0;
            const progressInterval = setInterval(() => {
            ...
            process.on('close', (code) => {
                ...
                if (code === 0 && !cancelled) {
                    ...
                    vscode.window.showInformationMessage(`仓库 ${repoName} 克隆成功!`, ...openOptions.map(opt => opt.label)).then(chosenOption => {
                        if (chosenOption) {
...
// 使用 exec 添加远程仓库
async function addRemoteWithExec(name, url, cwd) {
    return new Promise((resolve, reject) => {
 ...
 * @returns CSS样式内容
 */
function getStylesheetContent(context) {
 ...
 * @returns HTML字符串
 */
function createWebViewContent(title, content) {
...
// 修改创建WebView的方法
function createWebView(title, content) {
    const panel = vscode.window.createWebviewPanel('giteaWebView', title, vscode.ViewColumn.One, {
        enableScripts: true,
...
class CommandItem extends vscode.TreeItem {
    constructor(label, commandId, icon, iconColor, description, tooltipText) {
        super(label, vscode.TreeItemCollapsibleState.None);
        this.label = label;
...
class GiteaRepositoriesProvider {
    constructor() {
        ...
        // 订阅服务器变更事件
        this.disposables.push(this.serverManager.onServerChanged(() => {
            // 当服务器变更时刷新仓库列表
            this.refresh();
            // 延迟再次刷新以确保UI更新
...
单击删除图标可删除此仓库`;
    ...
    async getChildren(element) {
        ...
        try {
            const gitea = new giteaApi_1.GiteaAPI(currentServer.url, currentServer.token);
            ...
            return repos.map(repo => {
                ...
                return new Repository(repo.full_name, repo.description || '', repo.private ? 'private' : 'public', repoData);
            });
        }
        catch (error) {
...
class ServerItem extends vscode.TreeItem {
    constructor(label, isDefault, serverData) {
        super(label, vscode.TreeItemCollapsibleState.None);
        this.label = label;
        ...
        this.iconPath = new vscode.ThemeIcon(this.isDefault ? 'server-environment' : 'server', new vscode.ThemeColor(this.isDefault ? 'charts.green' : 'charts.blue'));
    }
}
// 服务器管理视图
class GiteaServersProvider {
    constructor() {
        this._onDidChangeTreeData = new vscode.EventEmitter();
        this.onDidChangeTreeData = this._onDidChangeTreeData.event;
        this.serverManager = serverManager_1.ServerManager.getInstance();
    ...
    async getChildren(element) {
        ...
        return servers.map(server => {
            return new ServerItem(server.name, server.isDefault || false, {
                name: server.name,
                url: server.url
            });
        });
...
async function activate(context) {
    ...
    const repoManager = repoManager_1.RepoManager.getInstance();
    const visibilityManager = visibilityManager_1.VisibilityManager.getInstance();
    // 注册命令
    ...
    let cloneRepoCommand = vscode.commands.registerCommand('mygit.cloneRepo', async (repoDataOrTreeItem) => {
    ...
    let switchServerCommand = vscode.commands.registerCommand('mygit.switchServer', async (serverItem) => {
        let serverName;
        // 如果是从服务器项上下文菜单调用
    ...
    let createPRCommand = vscode.commands.registerCommand('mygit.createPR', async () => {
        ...
        try {
            ...
            const gitea = new giteaApi_1.GiteaAPI(serverUrl, token);
    ...
    let initRepoCommand = vscode.commands.registerCommand('mygit.initRepo', async () => {
        try {
            ...
            // 确认是否初始化仓库
            const answer = await vscode.window.showWarningMessage(`是否将当前文件夹 "${workspaceFolder}" 初始化为Git仓库？`, { modal: true, detail: '这将在当前文件夹中创建Git仓库' }, { title: '是', isCloseAffordance: false });
            if (answer === undefined) {
                return;
            }
            // 初始化仓库
            ...
            if (success) {
                ...
                // 询问是否创建并关联远程仓库
                const addRemote = await vscode.window.showInformationMessage('是否创建并关联远程Gitea仓库？', '创建并关联', '手动关联', '稍后再说');
                if (addRemote === '创建并关联') {
                    // 检查Gitea配置
                    const server = serverManager.getCurrentServer();
                    if (!server) {
                        const configNow = await vscode.window.showErrorMessage('未配置Gitea服务器连接信息，是否现在配置？', '配置', '取消');
                        if (configNow === '配置') {
                    ...
                    try {
                        ...
                        // 提示用户手动关联
                        const manualConnect = await vscode.window.showInformationMessage('是否尝试手动关联远程仓库？', '手动关联', '取消');
    ...
    // 注册删除仓库命令
    let deleteRepoCommand = vscode.commands.registerCommand('mygit.deleteRepo', async (repoDataOrTreeItem) => {
        try {
            ...
            // 显示确认对话框
            const confirm = await vscode.window.showWarningMessage(`确定要删除仓库 "${selectedRepo.full_name}" 吗？此操作不可恢复！`, { modal: true, detail: '此操作不可撤销' }, { title: '删除', isCloseAffordance: false });
            if (confirm === undefined) {
                return;
            }
            // 再次确认
            const finalConfirm = await vscode.window.showWarningMessage(`请再次确认是否删除仓库 "${selectedRepo.full_name}"？`, { modal: true, detail: '此操作将永久删除此仓库及其所有内容' }, { title: '确认删除', isCloseAffordance: false });
    ...
    let showLogDisposable = vscode.commands.registerCommand('mygit.showLog', () => {
        (0, gitCommands_1.showGitLog)(context);
    });
    // 注册重置Git更改命令
    let resetDisposable = vscode.commands.registerCommand('mygit.reset', () => {
        (0, gitCommands_1.resetGitChanges)(context);
    });
    let manageServersCommand = vscode.commands.registerCommand('mygit.manageServers', async () => {
        ...
        if (action.value === 'add') {
            ...
            const serverToDelete = await vscode.window.showQuickPick(servers.map(server => ({
                label: server.name,
                description: server.url,
                detail: server.isDefault ? '当前服务器' : undefined,
                value: server.name
            ...
            const confirmed = await vscode.window.showWarningMessage(`确定要删除服务器 "${serverToDelete.label}" 吗？`, { modal: true }, '删除', '取消');
            if (confirmed === '删除') {
    ...
    // 注册仓库提示命令
    let showTooltipCommand = vscode.commands.registerCommand('_mygit.showTooltip', (repoName, visibility) => {
        // 仅显示提示，不执行任何操作
        // 这个命令允许我们在Repository类中设置command属性，而不会影响删除按钮的功能
    });
    // 添加到订阅列表
    context.subscriptions.push(reposTreeView, commandsTreeView, serversTreeView, refreshCommand, refreshServersCommand, connectCommand, createRepoCommand, cloneRepoCommand, switchServerCommand, pushCommand, pullCommand, commitCommand, listBranchesCommand, createBranchCommand, switchBranchCommand, createPRCommand, listPRsCommand, showStatusCommand, initRepoCommand, deleteRepoCommand, showLogDisposable, resetDisposable, manageServersCommand, showTooltipCommand);
}
