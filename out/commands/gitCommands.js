...
exports.resetGitChanges = exports.showGitLog = void 0;
const vscode = require("vscode");
const gitExtension_1 = require("../git/gitExtension");
/**
 * 显示Git提交历史日志
 * @param context VSCode扩展上下文
 */
async function showGitLog(context) {
    try {
        const git = new gitExtension_1.GitExtension();
 ...
 */
async function resetGitChanges(context) {
    try {
        const git = new gitExtension_1.GitExtension();
        // 1. 获取最近20条提交历史
        const log = await git.getLog();
        ...
        // 4. 最终确认
        const confirm = await vscode.window.showWarningMessage(`确定要将仓库重置到 ${selected.label} (${resetType.label}) 吗？此操作不可撤销。`, { modal: true }, '确定', '取消');
        if (confirm !== '确定') {
            return;
        }
        // 5. 执行reset
...
exports.resetGitChanges = resetGitChanges;
